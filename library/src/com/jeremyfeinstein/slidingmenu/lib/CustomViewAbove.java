...
public class CustomViewAbove extends ViewGroup {
	...
	/**
	 * To fix the issue https://github.com/jfeinstein10/SlidingMenu/issues/451
	 * */
	private boolean useNewIgnoredViewCheck = false;

	private boolean isInIgnoredView(MotionEvent ev) {
		if (useNewIgnoredViewCheck) {
			return isInIgnoredViewNew(ev);
		}
		Rect rect = new Rect();
		for (View v : mIgnoredViews) {
			...
			/**
			 * To fix the issue
			 * https://github.com/jfeinstein10/SlidingMenu/issues/451
			 * */
			// v.getHitRect(rect);
			v.getGlobalVisibleRect(rect);

			// Log.e("isInIgnoredView",
			// "isInIgnoredView - Rect :  " + rect.toString() + "");
			// Log.e("isInIgnoredView", "isInIgnoredView - EvX :  " + ev.getX()
			// + "   EvY :" + (int) ev.getY() + "");
			if (rect.contains((int) ev.getX(), (int) ev.getY())) {
				// Log.e("isInIgnoredView", "isInIgnoredView - " + true + "");
				return true;
			}
		}
		// Log.e("isInIgnoredView", "isInIgnoredView - " + false + "");
		return false;
	}

	private boolean isInIgnoredViewNew(MotionEvent ev) {
		for (View v : mIgnoredViews) {
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		mViewBehind.drawFadeMenu(mContent, canvas, getPercentOpen());
		mViewBehind.drawFadeAboveContent(mContent, canvas, getPercentOpen());
